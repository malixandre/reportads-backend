...
export interface CampaignData {
  ...
  ctr: string;
  metrics: Array<{ date: Date; views: number; clicks: number; ctr: number }>;
...
export class PdfService {
  ...
  private formatCtr(views: number, clicks: number): string {
    if (!views || views === 0) return '—';
    return ((clicks / views) * 100).toFixed(2) + '%';
  }

  private groupPrintsByDate(
    prints: CampaignData['prints'],
  ): Record<string, { MOBILE: string[]; DESKTOP: string[] }> {
  ...
  private buildChartDataScript(metrics: CampaignData['metrics']): string {
  ...
  private buildHeader(campaign: CampaignData): string {
  ...
  private buildDayPages(
    ...
    printsByDate: Record<string, { MOBILE: string[]; DESKTOP: string[] }>,
